...
# We need to have seperate ViTs for jeppa because they need to deal with masking
...
from experiment.models.SSLMethods.utils.tensors import (
    trunc_normal_,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if classification_head:
            ...
            self.class_token = nn.Parameter(torch.randn(1, 1, embed_dim))  # [1,1,embed]
            torch.nn.init.normal_(self.class_token, std=0.02)

        # --
        self.pos_embed = nn.Parameter(
            torch.zeros(1, num_patches + use_cls, embed_dim), requires_grad=False
        )
    ...
    def forward(self, x, masks=None):
        ...
        if self.classification_head:
            x = torch.cat(
                [self.class_token.expand(B, -1, -1), x], dim=1
            )  # cls now is [B, 1, embeddim] makes x = [B,N+1,]
