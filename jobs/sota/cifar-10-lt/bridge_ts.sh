#!/bin/sh
#SBATCH --job-name=sota-cifar-10-lt-bridge-ts
#SBATCH --output=job_logs/sota/cifar-10-lt/bridge_ts_%A_%a.out
...
mkdir -p job_logs/sota/cifar-10-lt
...
torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} -m experiment \
    ...
    experiment_name=sota_cifar-10-lt_bridge-ts \
