#!/bin/sh
#SBATCH --job-name=sota-imagenet-1k-bridge-ts
#SBATCH --output=job_logs/sota/imagenet-1k/bridge_ts_%A_%a.out
...
mkdir -p job_logs/sota/imagenet-1k
...
torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} -m experiment \
    ...
    experiment_name=sota_imagenet-1k_bridge-ts \
