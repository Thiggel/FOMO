#!/bin/sh
#SBATCH --job-name=sota-diffusiondb-subset-sdclr
#SBATCH --output=job_logs/sota/diffusiondb-subset/sdclr_%A_%a.out
...
torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} -m experiment \
    ...
    experiment_name=sota_diffusiondb-subset_sdclr \
