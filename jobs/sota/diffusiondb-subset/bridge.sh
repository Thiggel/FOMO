#!/bin/sh
#SBATCH --job-name=sota-diffusiondb-subset-bridge
#SBATCH --output=job_logs/sota/diffusiondb-subset/bridge_%A_%a.out
...
torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} -m experiment \
    ...
    experiment_name=sota_diffusiondb-subset_bridge \
