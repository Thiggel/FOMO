#!/bin/sh
#SBATCH --job-name=ablations-sample-selection-mode-window
#SBATCH --output=job_logs/ablations/sample_selection/mode_window_%A_%a.out
...
torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} -m experiment \
    ...
    experiment_name=ablations_sample-selection_mode-window \
    num_runs=3 \
