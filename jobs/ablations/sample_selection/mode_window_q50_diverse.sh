#!/bin/sh
#SBATCH --job-name=ablations-sample-selection-mode-window-q50-diverse
#SBATCH --output=job_logs/ablations/sample_selection/mode_window_q50_diverse_%A_%a.out
...
torchrun --standalone --nproc_per_node=${NPROC_PER_NODE} -m experiment \
    ...
    ood_mode_histogram_quantile_range=[0.50,0.99] \
    ...
    experiment_name=ablations_sample-selection_mode-window-q50-diverse \
    num_runs=3 \
