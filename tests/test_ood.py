...
def test_extract_features():
    ...
    assert torch.all(torch.eq(train_features, torch.tensor([[1, 2, 3], [4, 5, 6]])))
    assert torch.all(torch.eq(val_features, torch.tensor([[7, 8, 9], [10, 11, 12]])))
    
def test_ood():
    # Create dummy features
    train_features = torch.tensor([[0, 0, 1], [0, 1, 0]])
    val_features = torch.tensor([[0, 0, 0], [10, 5, 8]])
